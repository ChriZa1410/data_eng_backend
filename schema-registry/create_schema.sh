...
SCHEMA_REGISTRY_CONTAINER_PORT=8081
...
# Wait until schema-registry has started and kafka topics are initialized
...
curl -X POST -H "Content-Type: application/vnd.schemaregistry.v1+json" \
      ...
      http://schema-registry:$SCHEMA_REGISTRY_CONTAINER_PORT/subjects/topic_upload_data-value/versions

curl -X POST -H "Content-Type: application/vnd.schemaregistry.v1+json" \
      --data "$SCHEMA_AGGREGATED_DATA" \
      http://schema-registry:$SCHEMA_REGISTRY_CONTAINER_PORT/subjects/topic_aggregated_data-value/versions